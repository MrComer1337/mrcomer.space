document.onkeydown = function (e) {
    if ((e.ctrlKey && 85 == e.keyCode) || (e.ctrlKey && e.shiftKey && 73 == e.keyCode) || 123 == e.keyCode) return !1;
}),
...
var config = { TEXTURE_DOWNSAMPLE: 1, DENSITY_DISSIPATION: 0.98, VELOCITY_DISSIPATION: 0.99, PRESSURE_DISSIPATION: 0.8, PRESSURE_ITERATIONS: 25, CURL: 30, SPLAT_RADIUS: 0.005 },
    pointers = [],
    splatStack = [],
...
function getWebGLContext(e) {
    var t = { alpha: !1, depth: !1, stencil: !1, antialias: !1 },
        n = e.getContext("webgl2", t),
        r = !!n;
    r || (n = e.getContext("webgl", t) || e.getContext("experimental-webgl", t));
    var i = n.getExtension("OES_texture_half_float"),
        o = n.getExtension("OES_texture_half_float_linear");
    return (
        r && (n.getExtension("EXT_color_buffer_float"), (o = n.getExtension("OES_texture_float_linear"))),
        n.clearColor(0, 0, 0, 1),
        { gl: n, ext: { internalFormat: r ? n.RGBA16F : n.RGBA, internalFormatRG: r ? n.RG16F : n.RGBA, formatRG: r ? n.RG : n.RGBA, texType: r ? n.HALF_FLOAT : i.HALF_FLOAT_OES }, support_linear_float: o }
    );
}
...
var GLProgram = (function () {
    function e(t, n) {
        ...
        if (((this.uniforms = {}), (this.program = gl.createProgram()), gl.attachShader(this.program, t), gl.attachShader(this.program, n), gl.linkProgram(this.program), !gl.getProgramParameter(this.program, gl.LINK_STATUS)))
...
function compileShader(e, t) {
    ...
    if ((gl.shaderSource(n, t), gl.compileShader(n), !gl.getShaderParameter(n, gl.COMPILE_STATUS))) throw gl.getShaderInfoLog(n);
    return n;
}
...
function initFramebuffers() {
    ...
    var e = ext.internalFormat,
        t = ext.internalFormatRG,
        n = ext.formatRG,
        r = ext.texType;
    (density = createDoubleFBO(0, textureWidth, textureHeight, e, gl.RGBA, r, support_linear_float ? gl.LINEAR : gl.NEAREST)),
        (velocity = createDoubleFBO(2, textureWidth, textureHeight, t, n, r, support_linear_float ? gl.LINEAR : gl.NEAREST)),
        (divergence = createFBO(4, textureWidth, textureHeight, t, n, r, gl.NEAREST)),
        (curl = createFBO(5, textureWidth, textureHeight, t, n, r, gl.NEAREST)),
        (pressure = createDoubleFBO(6, textureWidth, textureHeight, t, n, r, gl.NEAREST));
}
function createFBO(e, t, n, r, i, o, a) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, s),
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, r, t, n, 0, i, o, null);
    var u = gl.createFramebuffer();
    return gl.bindFramebuffer(gl.FRAMEBUFFER, u), gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, s, 0), gl.viewport(0, 0, t, n), gl.clear(gl.COLOR_BUFFER_BIT), [s, u, e];
}
function createDoubleFBO(e, t, n, r, i, o, a) {
    var s = createFBO(e, t, n, r, i, o, a),
        u = createFBO(e + 1, t, n, r, i, o, a);
    return {
        get first() {
            return s;
        },
...
function update() {
    ...
    if (((lastTime = Date.now()), gl.viewport(0, 0, textureWidth, textureHeight), splatStack.length > 0))
        for (var t = 0; t < splatStack.pop(); t++) {
            var n = [10 * Math.random(), 10 * Math.random(), 10 * Math.random()];
            splat(canvas.width * Math.random(), canvas.height * Math.random(), 1e3 * (Math.random() - 0.5), 1e3 * (Math.random() - 0.5), n);
        }
    advectionProgram.bind(),
        ...
        density.swap();
    for (var r = 0, i = pointers.length; r < i; r++) {
        var o = pointers[r];
    ...
    curlProgram.bind(),
        ...
        blit(divergence[1]),
        clearProgram.bind();
    var a = pressure.first[2];
    gl.activeTexture(gl.TEXTURE0 + a),
        gl.bindTexture(gl.TEXTURE_2D, pressure.first[0]),
        gl.uniform1i(clearProgram.uniforms.uTexture, a),
        ...
        (a = pressure.first[2]),
        gl.activeTexture(gl.TEXTURE0 + a);
    for (var s = 0; s < config.PRESSURE_ITERATIONS; s++) gl.bindTexture(gl.TEXTURE_2D, pressure.first[0]), gl.uniform1i(pressureProgram.uniforms.uPressure, a), blit(pressure.second[1]), pressure.swap();
...
function splat(e, t, n, r, i) {
    splatProgram.bind(),
        gl.uniform1i(splatProgram.uniforms.uTarget, velocity.first[2]),
        gl.uniform1f(splatProgram.uniforms.aspectRatio, canvas.width / canvas.height),
        gl.uniform2f(splatProgram.uniforms.point, e / canvas.width, 1 - t / canvas.height),
        gl.uniform3f(splatProgram.uniforms.color, n, -r, 1),
        gl.uniform1f(splatProgram.uniforms.radius, config.SPLAT_RADIUS),
        blit(velocity.second[1]),
        velocity.swap(),
        gl.uniform1i(splatProgram.uniforms.uTarget, density.first[2]),
        gl.uniform3f(splatProgram.uniforms.color, 0.3 * i[0], 0.3 * i[1], 0.3 * i[2]),
        blit(density.second[1]),
        density.swap();
}
...
function renderTime() {
    var e = new Date(),
        ...
        r = e.getMinutes(),
        i = e.getSeconds();
    setTimeout("renderTime()", 1e3), 0 == n ? (n = 12) : n > 12 && ((n -= 12), (t = "PM")), n < 10 && (n = "0" + n), r < 10 && (r = "0" + r), i < 10 && (i = "0" + i);
    document.getElementById("clockDisplay");
    ((e = document.querySelectorAll(".hover-ch"))[0].textContent = n), (e[1].textContent = r), (e[2].textContent = i), (e[3].textContent = t);
}
document.addEventListener("mousemove", function (e) {
    ++count > 25 && ((colorArr = [Math.random() + 0.2, Math.random() + 0.2, Math.random() + 0.2]), (count = 0)),
        ...
        (pointers[0].dx = 10 * (e.clientX - pointers[0].x)),
        (pointers[0].dy = 10 * (e.clientY - pointers[0].y)),
    ...
    document.addEventListener(
        ...
        function (e) {
            ...
            for (var n = 0, r = t.length; n < r; n++) {
                n >= pointers.length && pointers.push(new pointerPrototype()), (pointers[n].id = t[n].identifier), (pointers[n].down = !0), (pointers[n].x = t[n].pageX), (pointers[n].y = t[n].pageY), (pointers[n].color = colorArr);
                var i = pointers[n];
                (i.moved = i.down), (i.dx = 10 * (t[n].pageX - i.x)), (i.dy = 10 * (t[n].pageY - i.y)), (i.x = t[n].pageX), (i.y = t[n].pageY);
            }
        },
        !1
    ),
...
function pVid() {
    const e = document.querySelector("#pp-btn");
    audio.paused ? (e.classList.remove("fa-play"), e.classList.add("fa-pause"), audio.play()) : (e.classList.remove("fa-pause"), e.classList.add("fa-play"), audio.pause());
}
...
(dt.innerHTML = ""),
    tc.split("").forEach((e) => {
        ...
        t.classList.add("hover-effect"), (t.textContent = e), dt.appendChild(t);
    });
